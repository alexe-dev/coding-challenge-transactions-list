...
const SingleTransaction: React.FC<SingleTransactionProps> = ({ id }) => {
  ...
  const { loading, error, data } = useQuery<SingleTransactionData>(GetSingleTransaction, { variables: { hash: id } });

  if (loading) {
    return (
      <div className="flex flex-col mt-20">
        <div className="max-w-[85rem] w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between">Loading...</div>
      </div>
    );
  }
